...
public partial class EditTaskPage : ContentPage, INotifyPropertyChanged
{
    private TodoItem? _originalItem; // Изменено: добавлен знак вопроса ?
    ...
    public EditTaskPage(TodoItem itemToEdit)
    {
        InitializeComponent();
        
        if (itemToEdit == null)
        {
            DisplayAlert("Ошибка", "Не удалось загрузить задачу для редактирования", "OK");
            Navigation.PopAsync();
            return;
        }
        ...
        // Заполняем поля значениями из редактируемой задачи
        TitleEntry.Text = itemToEdit.Title;
        DescriptionEditor.Text = itemToEdit.Description;
        SelectedPriority = itemToEdit.Priority;
        selectedDate = itemToEdit.Date;
        selectedTime = itemToEdit.Time;
        reminderMinutes = itemToEdit.ReminderMinutes;
    ...
    private async void OnEditTaskConfirm(object sender, EventArgs e)
    {
        var titleText = TitleEntry?.Text?.Trim() ?? string.Empty;
        var desc = DescriptionEditor?.Text?.Trim();

        // ПРОВЕРКА: если пользователь убрал или забыл ввести название
        if (string.IsNullOrWhiteSpace(titleText))
        {
            await DisplayAlert("Внимание", "Нужно указать название задачи", "OK");
            return;
        }

        // ПРОВЕРКА: что оригинальный элемент существует (добавлено)
        if (_originalItem == null)
        {
            await DisplayAlert("Ошибка", "Не удалось найти задачу для обновления", "OK");
            await Navigation.PopAsync();
            return;
        }

        // Обновляем оригинальный объект
        _originalItem.Title = titleText;
        _originalItem.Description = desc;
        _originalItem.Priority = SelectedPriority;
        _originalItem.Date = selectedDate;
        _originalItem.Time = selectedTime;
        _originalItem.ReminderMinutes = reminderMinutes;
        
        _originalItem.UpdateDateDisplay();

        // Обновляем напоминания если нужно
        if (reminderMinutes.HasValue && selectedDate.HasValue && selectedTime.HasValue)
        {
            ...
            Console.WriteLine($"Обновлено напоминание для '{titleText}' в {reminderTime:HH:mm}");
        }

        await DisplayAlert("Готово", "Задача обновлена", "OK");
        await Navigation.PopAsync();
    }
